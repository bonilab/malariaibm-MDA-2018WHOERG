...
#            ax2.plot(data.index, data[.5],'--')
    ...
    ax[idx,0].text(dt.date(2020, 6, 1), 2.25, figure_index[idx*3], fontsize=36, weight='bold')
...
data= pd.read_csv('data/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL%s_pfpr.csv'%(4,2, "" ), sep=',', header=None)
data = data.quantile([0.05, 0.25, 0.5, 0.75, 0.95], axis=1).T                        
data.index = dates