...
#    
    for i_mda, mda in enumerate(mdas):  
        freq_580Y_plas2 =  pd.read_csv('data_40k/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL%s_580Y_plas2.csv'%(mda,pfpr,scenario), sep=',', header=None)
        freq_580Y= pd.read_csv('data_40k/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL%s_C580Y.csv'%(mda,pfpr,scenario), sep=',', header=None)
        freq_plas2= pd.read_csv('data_40k/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL%s_plas.csv'%(mda,pfpr,scenario), sep=',', header=None)
...
#        ax[idx,0].set_title('PfPR_2-10')
    ...
    if(idx==3):
        ...
        ax[idx,3].set_xticklabels(range(0,19,5))
...
#    print(ytick_labels)
    ...
    ax[idx,0].text(dt.date(2020, 6, 1), 0.45, figure_index[idx*3], fontsize=32, weight='bold')
    ax[idx,1].text(dt.date(2020, 6, 1), 0.45, figure_index[idx*3+1], fontsize=32, weight='bold')
    ax[idx,2].text(dt.date(2020, 6, 1), 0.45, figure_index[idx*3+2], fontsize=32, weight='bold')
...
#%%
ax[0,3].text(dt.date(2020, 6, 1), 0.001, figure_index[0*3+3], fontsize=32, weight='bold')
ax[1,3].text(dt.date(2020, 6, 1), 0.75, figure_index[1*3+3], fontsize=32, weight='bold')
ax[2,3].text(dt.date(2020, 6, 1), -0.001, figure_index[2*3+3], fontsize=32, weight='bold')
ax[3,3].text(dt.date(2020, 6, 1), 0.7, figure_index[3*3+3], fontsize=32, weight='bold')


# ax[0,3].legend(custom_lines, ['0', '1', '2','3','4'], title="# MDA ROUNDS", loc="lower right")
