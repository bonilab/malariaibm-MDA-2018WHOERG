...
plt.close('all')
fig, ax = plt.subplots(2,3, figsize=(fig_w, fig_h),tight_layout=True)
...
for idx, scenario in enumerate(scenarios):
    ...
    for i_mda, mda in enumerate(mdas):        
        data= pd.read_csv('data/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL_imp_OZ%s_pfpr.csv'%(mda,pfpr,scenario ), sep=',', header=None)
        ...
        data= pd.read_csv('data/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL_imp_OZ%s_positive.csv'%(mda,pfpr,scenario), sep=',', header=None)
        data = data.fillna(0)
        data = data.quantile([0.05, 0.25, 0.5, 0.75, 0.95], axis=1).T        
        data.index = dates
#        ax[idx,1].fill_between(data.index, data[.25], data[.75], alpha=.2)
        ...
        data= pd.read_csv('data/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL_imp_OZ%s_C580Y.csv'%(mda,pfpr,scenario), sep=',', header=None)
        data = data.fillna(0)
        data = data.quantile([0.05, 0.25, 0.5, 0.75, 0.95], axis=1).T        
        data.index = dates
#        ax[idx,1].fill_between(data.index, data[.25], data[.75], alpha=.2)
...
#            ax2.plot(data.index, data[.5],'--')
        ...
        data= pd.read_csv('data/ONELOC_40k_%dRMDA_PFPR%d_OPPUNIFORM_FLAL_imp_OZ%s_plas.csv'%(mda,pfpr,scenario), sep=',', header=None)
        data = data.fillna(0)
        data = data.quantile([0.05, 0.25, 0.5, 0.75, 0.95], axis=1).T
        data.index = dates
        #        ax[idx,1].fill_between(data.index, data[.25], data[.75], alpha=.2)
