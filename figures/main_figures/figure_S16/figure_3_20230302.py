# -*- coding: utf-8 -*-
"""
Created on Fri Aug 31 14:03:19 2020
...
#        ax[idx,1].fill_between(data.index, data[.25], data[.75], alpha=.2)
        ...
        data.index = dates
#        ax[idx,1].fill_between(data.index, data[.25], data[.75], alpha=.2)
        ax[idx,1].plot(data.index, data[.5])
        ...
        data.index = dates
        #        ax[idx,1].fill_between(data.index, data[.25], data[.75], alpha=.2)
        ax[idx,2].plot(data.index, data[.5])
    
    ax[idx,0].text(dt.date(2020, 6, 1), 7.25, figure_index[idx*3], fontsize=36, weight='bold')
    ax[idx,1].text(dt.date(2020, 6, 1), 0.45, figure_index[idx*3+1], fontsize=36, weight='bold')
    ax[idx,2].text(dt.date(2020, 6, 1), 0.45, figure_index[idx*3+2], fontsize=36, weight='bold')
   
    ax21.grid(False)
    ax22.grid(False)    
    ax21.set_ylim(100, 100000)
#        ax21.yaxis.set_ticks(np.arange(0, 4000, 1000))
    ax22.set_ylim(100, 100000)
    ax21.yaxis.set_ticks([  100, 1000, 10000, 100000])
    ax22.yaxis.set_ticks([ 100, 1000, 10000, 100000])
        
    
    
    ax[idx,0].set_ylim(0, 8)
    ax[idx,0].set_xlim(dt.date(2020, 1, 1), dt.date(2042, 1, 1))
    ax[idx,0].tick_params(axis='both', which='both')
    ax[idx,0].yaxis.set_major_locator(plt.MaxNLocator(4))
...
#    ax[idx,0].set_ylabel(r'$PFPR_{2-10}$',fontsize= fs)
    ...
    ax[idx,1].yaxis.set_major_formatter(ticker.ScalarFormatter(useMathText=True))
...
#    ax[idx,1].set_ylabel('580Y Frequency',fontsize= fs)
    ...
    ax[idx,2].yaxis.set_major_formatter(ticker.ScalarFormatter(useMathText=True))
