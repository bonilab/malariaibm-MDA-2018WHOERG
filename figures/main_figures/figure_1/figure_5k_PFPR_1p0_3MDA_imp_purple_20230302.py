...
def make_plot(pfpr, pfpr_str, scenario, scenario_str, mda):     
    ...
    data_positive= pd.read_csv('data\ONELOC_5k_%dRMDA_PFPR%s_OPPUNIFORM_FLAL%s_positive.csv'%(mda,pfpr_str,scenario ), sep=',', header=None)
...
fig = plt.gcf()
fig.savefig("purple.pdf")
# #%%
# i=0
# for pfpr, pfpr_str in pfprs.items():
#     for scenario, scenario_str in scenarios.items():
#         for mda in mdas:
#             figs[i].savefig('pos_indivs_5k_PFPR%s_%dMDA%s.pdf'%(pfpr_str, mda,scenario))
#             i+=1
